...
var Board = function() {
		...
		if (piece instanceof Pawn) {
			piece.getMovePaths().forEach(function(move) {
				...
				if (move.column == piece.currentPosition.column && move.row < piece.currentPosition.row) {
					for (let i = piece.currentPosition.row - 1; i >= move.row ; i--) {
	...
	return {
		initializeBoard: function() {
			// White Pawns
			for(var i = 0; i <= 7; i++) {
				let pawn = new Pawn('w', {row: 6, column: i});
				pawn.setIcon();
				board[6][i] = pawn;
			}
			// Black Pawns
			for(i = 0; i <= 7; i++) {
				let pawn = new Pawn('b', {row: 1, column: i});
				pawn.setIcon();
				board[1][i] = pawn;
			}
			// White Rooks
			let rook = new Rook('w', {row: 7, column: 0});
			rook.setIcon();
			board[7][0] = rook;
			...
			board[0][7] = rook;
			// White Knights
			let knight = new Knight('w', {row: 7, column: 1});
			knight.setIcon();
			board[7][1] = knight;
			...
			board[0][6] = knight;
			// White Bishops
			let bishop = new Bishop('w', {row: 7, column: 2});
			bishop.setIcon();
			board[7][2] = bishop;
			...
			board[0][5] = bishop;
			// White Queen
			let queen = new Queen('w', {row: 7, column: 3});
			queen.setIcon();
			board[7][3] = queen;
			// Black Queen
			...
			board[0][3] = queen;
			// White King
			let king = new King('w', {row: 7, column: 4});
			king.setIcon();
			board[7][4] = king;
			// Black King
